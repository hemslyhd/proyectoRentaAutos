...
void Bienvenida::on_pushButtonEntrar_clicked()
{
    if(ui->lineEditNombreEmpresa->text()!=""){
        int opcion=QMessageBox::question(this,"Confirmar?"," El nombre de la empresa es "+ui->lineEditNombreEmpresa->text()+" y la cantidad de dinero inicial es "+ QString::number( ui->spinBoxCantDinero->value()),QMessageBox::Yes|QMessageBox::No);
        if(opcion==QMessageBox::Yes){
            ...
            //cant=ui->spinBoxCantAutos->value();
            dinero=ui->spinBoxCantDinero->value();
            controladorPantallaBienvenida.agregarNombreDineroEmpresa(nombre,dinero);
